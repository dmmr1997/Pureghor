...
export const StoreProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const shippingFee =
    cartSubtotal >= (storeSettings.freeDeliveryThreshold || 1500)
      ? 0
      : deliveryZone === 'inside_dhaka'
      ? (storeSettings.deliveryChargeInsideDhaka ?? 70)
      : (storeSettings.deliveryChargeOutsideDhaka ?? 130);

  const cartTotal = Math.max(0, cartSubtotal - (couponDiscount || 0) + shippingFee);
  ...
  const addToCart = (
    ...
    const price = Number(selectedVariant ? selectedVariant.price : product.price) || 0;
    const compareAtPrice = selectedVariant ? selectedVariant.compareAtPrice : product.compareAtPrice;
    const weight = selectedVariant?.weight || product.weight || '500 গ্রাম';
    const sku = selectedVariant?.sku || product.sku || `PG-${product.id}`;
    
    // Robust stock determination
    const rawStock = selectedVariant?.stock ?? product.stock;
    const maxStock = typeof rawStock === 'number' && !isNaN(rawStock) && rawStock > 0 ? rawStock : 99;
    const safeQty = typeof quantity === 'number' && !isNaN(quantity) && quantity > 0 ? Math.min(quantity, maxStock) : 1;

    setCart(prev => {
      const targetVariantId = selectedVariant?.id;
      const existingIndex = prev.findIndex(
        item => item.productId === product.id && (item.variantId || undefined) === (targetVariantId || undefined)
      );
      ...
      if (existingIndex > -1) {
        ...
        if (newQty > maxStock) {
          ...
          addToast(
            language === 'bn'
              ? `"${product.nameBn}" এর পরিমাণ আপডেট হয়েছে`
              : `"${product.nameEn}" quantity updated in cart`,
            'success'
          );
        }
        return updated;
        ...
        const newItem: CartItem = {
          ...
          nameEn: product.nameEn || product.nameBn,
          price,
          compareAtPrice,
          quantity: safeQty,
          weight,
          image: product.mainImage || (product.images && product.images[0]) || '/images/products/pureghor_blackseed_honey_1787810945841.jpg',
          maxStock,
          sku,
        };
        addToast(
          language === 'bn'
            ...
            : `"${product.nameEn || product.nameBn}" added to cart successfully`,
          'success'
        );
        return [...prev, newItem];
      }
    ...
    setCart(prev =>
      prev.map(item => {
        if (item.productId === productId && (item.variantId || undefined) === (variantId || undefined)) {
          const itemMaxStock = typeof item.maxStock === 'number' && !isNaN(item.maxStock) && item.maxStock > 0 ? item.maxStock : 99;
          if (newQty > itemMaxStock) {
            addToast(
              language === 'bn'
                ? `সর্বোচ্চ স্টক ${itemMaxStock}টি উপলব্ধ`
                : `Maximum available stock is ${itemMaxStock}`,
              'info'
            );
            return { ...item, quantity: itemMaxStock };
          }
