...
import { Product, Category, Banner, Coupon, Order, StoreSettings, CustomerUser, AdminUser, Review, DashboardStats } from './types';
...
export async function startServer() {
  ...
  app.post('/api/upload', (req: Request, res: Response) => {
    try {
      ...
      // If it's a data URL, save it as a physical file in /public/uploads
      const matches = image.match(/^data:([A-Za-z-+\/]+);base64,(.+)$/);
      if (matches && matches.length === 3) {
        const ext = matches[1].split('/')[1] || 'jpg';
        const safeExt = ext.replace('jpeg', 'jpg');
        const cleanName = filename ? filename.replace(/\.[^/.]+$/, '') : `upload_${Date.now()}`;
        const finalFilename = `${cleanName}_${Date.now()}.${safeExt}`;
